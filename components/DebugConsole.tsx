...
export default function DebugConsole({
	...
	const getStatusText = () => {
		switch (status) {
			case 'quota_error':
				return 'QUOTA LIMIT - WAITING';
			case 'retry':
				return 'RESUMING TRANSLATION';
			default:
							...
							{showTooltip && (
								<div className="absolute left-6 top-0 z-10 w-72 p-3 bg-gray-800 border border-yellow-600 rounded-lg shadow-lg text-xs">
									...
									<div className="text-gray-200 mb-2">
										Free Gemini API has a limit of 10 requests/minute. With a
										paid API key, there's no wait time!
									</div>
							...
							<div className="bg-yellow-800/40 rounded p-2">
								...
								<div className="w-full bg-yellow-800 rounded-full h-1 mt-2">
									<div
										...
										style={{
											width: `${Math.max(0, 100 - (timeLeft / (retryAfter || 65)) * 100)}%`,
										}}
									/>
								</div>
							</div>
							<div className="text-yellow-300 text-xs">
								💡 With a paid API, there are no limits and translation wouldn't
								be interrupted
							</div>
						</div>
					</div>
							...
							{totalChunks && (
								<div className="flex justify-between">
									...
									<span className="text-purple-400">
										~{Math.max(0, totalChunks - (currentChunk || 0))} chunks
										left
									</span>
								</div>
							)}
